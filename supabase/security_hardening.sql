...
-- 10.【可选 · 需你确认】清理可疑账号
--
-- 2026-09-14 10:00 前后（北京时间）出现两个账号：
--   · sectest_0914   role=owner   09-14 09:59:53 被创建/提权
--   · testuser001    role=user    09-14 10:00:44
-- 如果不是你自己建的测试号，就是对方留下的后门 —— 取消下面注释后执行。
-- （删除 auth.users 会级联删掉 profiles 里的对应行）
-- ============================================================
-- delete from auth.users u using public.profiles p
--   where u.id = p.id and p.username in ('sectest_0914', 'testuser001');

-- 若只想降权、不想删号：
-- update public.profiles set role = 'user' where username = 'sectest_0914';
