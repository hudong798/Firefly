...
begin
  ...
  insert into public.site_profile (
    id, name, tagline, tags, manifesto, about_text, avatar_url,
    interests, freex_intro, contacts, updated_at
  ) values (
    1, p_name, p_tagline, p_tags, p_manifesto, p_about_text, p_avatar_url,
    p_interests, p_freex_intro, p_contacts, now()
  )
...
grant execute on function public.admin_save_site_profile(text, text, text, text, text[], text[], text, text, jsonb, text, jsonb) to anon, authenticated;
